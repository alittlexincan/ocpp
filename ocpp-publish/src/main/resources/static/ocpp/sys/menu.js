...
layui.use(["table","form","laytpl","layer","selectTree"], function(){
    ...
    let levelFormat = function(d){
        if(d.level == 1) return "<span class='layui-btn layui-btn-warm layui-btn-xs ocpp-cursor-default'>一级菜单</span>";
        if(d.level == 2) return "<span class='layui-btn layui-btn-normal layui-btn-xs ocpp-cursor-default'>二级菜单</span>";
        if(d.level == 3) return "<span class='layui-btn layui-btn-xs ocpp-cursor-default'>三级菜单</span>";
    };
