...
layui.use(['table','form','laytpl','layer', 'ajaxFileUpload'], function(){
     ...
     */
    let typeFormat = d => {
        if(d.type == 0) return "<span class='layui-btn layui-btn-xs layui-btn-warm ocpp-cursor-default'>渠道</span>";
        if(d.type == 1) return "<span class='layui-btn layui-btn-normal layui-btn-xs ocpp-cursor-default'>手段</span>";
    };
            ...
            layer.open({
                ...
                ,yes: function(index, layero){
                    ...
                    form.on("submit(submitAddBtn)", function(data){
                        ...
                        let iconIndex = icon.lastIndexOf("\\")+1;
        ...
        ,'deleteOption': function (obj) {
            layer.confirm('确定删除该发布手段？', function(index){
                ...
                submit({
                    ...
                    ,type: 'POST'
                    ,url: '/channel/delete/' + obj.data.id,
                });
            });
        }
        /**
            ...
            layer.open({
                ...
                ,success: function(layero,index){
                    ...
                    laytpl(updatePop.innerHTML).render(param, function(html){
                        ...
                        // 是否启用赋值
                        $("#updateDiv input[type='radio'][name='status'][value='"+param.status+"']").prop("checked", true);
                        form.render();
                    });
