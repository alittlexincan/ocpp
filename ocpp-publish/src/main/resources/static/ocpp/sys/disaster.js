...
layui.use(['table','form','laytpl','layer', 'ajaxFileUpload', 'selectTree', 'zTree', 'disaster'], function(){
    ...
    let typeFormat = function(d){
        if(d.type == -1) return "<span class='layui-btn layui-btn-xs layui-btn-normal ocpp-cursor-default'>根节点</span>";
        if(d.type == 0) return "<span class='layui-btn layui-btn-xs layui-btn-warm ocpp-cursor-default'>事件</span>";
        if(d.type == 1) return "<span class='layui-btn layui-btn-normal layui-btn-xs ocpp-cursor-default'>灾种</span>";
    };
    ...
    let iconFormat = function(d){
        if(d.icon!="" && d.icon!=null) {
            return "<img src='"+d.icon+"'  style='width:50px;height:50px;' >";
        }else{
            return "暂无图片";
        }
    };
    ...
    let active = {
        ...
        ,'updateBarBtn': function (obj) {
            // 获取灾种树选中节点
            let nodes = disasterZtree.getCheckedNodes(true);
            if(nodes.length == 0){
        ...
        ,'deleteLevelBarBtn': function(){
            ...
            for(let i = 0, len = data.length; i<len; i++){
                ...
                let iconIndex = (icon.lastIndexOf("\\")+1)||(icon.lastIndexOf("/")+1);
        ...
        ,'deleteOption': function (obj) {
            layer.confirm('确定删除该灾种？', function(index){
                ...
                submit({
                    ...
                    ,type: 'POST'
                    ,url: '/disaster/delete/' + obj.data.id,
                });
            });
        }
        /**
